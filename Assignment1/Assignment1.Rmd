...
```
Plugging the available data into the forcasted model is the basics approach that is conducted to obtain forecasted values. However, there are various methods to improve the forecast. 

We used the function \textit{predict()} to forecast the next 8 quarters based on the AR(1) model. It gives us a time series of predictions, using finite-history prediction via the Kalman filter. Note it is only statistically significant if the MA part is stationary (as an AR model does not have a MA part, this is not an issue for us). 
